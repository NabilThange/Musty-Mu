...
export default function PYQsPage() {
  ...
  const filteredPYQs = pyqsData
    ? pyqsData.filter((pyq) => pyq.subject_code?.toLowerCase().includes(searchQuery.toLowerCase()))
    : []
        ...
        <div className="container flex h-20 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center">
              ...
              <span className="font-black text-xl tracking-tighter">PREVIOUS YEAR QUESTIONS</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="space-y-8">
          ...
          <div className="text-center space-y-4">
            <h1 className="text-4xl md:text-6xl font-black tracking-tighter uppercase">📄 PYQ PAPERS</h1>
          ...
          {/* Error State */}
          {error && <ErrorCard message={error.message || "Failed to load PYQ data"} onRetry={refetch} />}
          ...
          {!isLoading && !error && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPYQs.map((pyq) => (
                <ResourceCard key={pyq.id} type="pyq" data={pyq} />
              ))}
            </div>
          )}

          {/* Empty State */}
          {!isLoading && !error && filteredPYQs.length === 0 && (
            <div className="text-center py-16 bg-gray-100 border-4 border-dashed border-black">
              <FileText className="h-16 w-16 mx-auto mb-4 text-gray-400" />
              <h3 className="text-2xl font-black mb-2">NO PYQs FOUND</h3>
              <p className="font-bold text-gray-600 mb-6">
                {searchQuery
                  ? `No PYQs match "${searchQuery}"`
                  : `No PYQs available for ${academicInfo.branch} Semester ${academicInfo.semester}`}
              </p>
              <Button
